...
# Thoda wait karna taaki Flask properly start ho jaye
time.sleep(2)
...
async def start_with_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) > 0:
        ...
        if folder_result:
            ...
            # Get the folder owner's settings for auto-delete info
            cursor.execute('SELECT user_id FROM folders WHERE folder_id = ?', (folder_id,))
            folder_owner_id = cursor.fetchone()[0]
            owner_settings = get_user_settings(folder_owner_id)
            ...
            if items:
                ...
                # Send auto-delete warning message at the end
                if owner_settings and owner_settings['auto_delete']:
                    auto_delete_time = owner_settings['auto_delete_time']
                    warning_msg = await update.message.reply_text(
                        f"⚠️ Important:\n\nAll Messages will be deleted after {auto_delete_time} minutes. "
                        "Please save or forward these messages to your personal saved messages to avoid losing them!"
                    )
                    
                    # Also auto-delete the warning message
                    if settings and settings['auto_delete']:
                        auto_delete_time_seconds = settings['auto_delete_time'] * 60
                        await asyncio.sleep(auto_delete_time_seconds)
                        try:
            ...
            cursor.execute(
                'SELECT message_type, content, file_id, protect_content, user_id FROM messages WHERE link_code = ?',
                (link_code,)
            )
            ...
            if message_result:
                message_type, content, file_id, protect_content, message_owner_id = message_result
                
                # Get the message owner's settings for auto-delete info
                owner_settings = get_user_settings(message_owner_id)
                ...
                if owner_settings and owner_settings['auto_delete']:
                    auto_delete_time = owner_settings['auto_delete_time']
                    warning_msg = await update.message.reply_text(
                        f"⚠️ Important:\n\nThis message will be deleted after {auto_delete_time} minutes. "
                        "Please save or forward it to your personal saved messages to avoid losing it!"
                    )
...
async def settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "setting_caption":
        ...
        update_user_settings(user_id, 'custom_caption', '')
        await query.edit_message_text("✅ Custom caption has been removed.")
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(settings_callback, pattern="^setting_|^remove_channel_|^add_channel|^settings_back|^check_force_sub|^remove_caption$"))
